...
PROMPT_V1_NAME = "huynh-day22-rag-prompt-v1"
PROMPT_V2_NAME = "huynh-day22-rag-prompt-v2"
...
SYSTEM_V2 = (
    ...
    "rồi trả lời có cấu trúc trong 3-5 câu. Nêu câu trả lời chính trước, sau đó bổ sung "
    "chi tiết hỗ trợ từ context và mức độ chắc chắn nếu cần. Không suy đoán ngoài context.\n\n"
    "Context:\n{context}"
)
...
def push_prompts_to_hub(client: Client):
    ...
    """
    try:
        url = client.push_prompt(PROMPT_V1_NAME, object=PROMPT_V1, description="V1 - ngan gon")
        print(f"✅ Đã push V1 → {url}")
    except Exception as e:
        print(f"⚠️  V1 lỗi: {e}")

    try:
        url = client.push_prompt(PROMPT_V2_NAME, object=PROMPT_V2, description="V2 - co cau truc")
...
def main():
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS):
        ...
        if config.REQUEST_DELAY_SECONDS and i + 1 < len(SAMPLE_QUESTIONS):
